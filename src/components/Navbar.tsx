...
export default function Navbar() {
    ...
    return (
        <>
            ...
            {/* Mobile Menu — rendered outside <nav> so it truly covers the full viewport */}
            <AnimatePresence>
                {mobileMenuOpen && (
                    <motion.div
                        initial={{ x: "100%" }}
                        ...
                        className="fixed inset-0 z-[60] bg-navy-dark flex flex-col pt-24 px-8 lg:hidden overflow-y-auto"
                    >
                        {/* Close button inside menu */}
                        <button
                            onClick={() => setMobileMenuOpen(false)}
                            className="absolute top-5 right-6 text-white hover:text-golden transition-colors"
                        ...
                        <div className="flex flex-col space-y-6 text-2xl font-heading">
                            <Link href="/" onClick={() => setMobileMenuOpen(false)} className="text-white hover:text-golden transition-colors">Home</Link>
                            <Link href="/about" onClick={() => setMobileMenuOpen(false)} className="text-white hover:text-golden transition-colors">About Us</Link>
                            <Link href="/services" onClick={() => setMobileMenuOpen(false)} className="text-white hover:text-golden transition-colors">Services</Link>
                            <Link href="/case-studies" onClick={() => setMobileMenuOpen(false)} className="text-white hover:text-golden transition-colors">Case Studies</Link>
                            <Link href="/blog" onClick={() => setMobileMenuOpen(false)} className="text-white hover:text-golden transition-colors">Blog</Link>
                            <Link href="/contact" onClick={() => setMobileMenuOpen(false)} className="text-white hover:text-golden transition-colors">Contact</Link>
                            <Link
                                href="/contact"
                                onClick={() => setMobileMenuOpen(false)}
